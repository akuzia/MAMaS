...
if (_endMissionToggle == 0) exitWith {MAMaS_endMission = {}};
...
MAMaS_processorEND = {
	...
	_toRPT set [count _toRPT,"MAMaS Unit states:"];
	{
...
MAMaS_endMission = {
	...
	if (count(_this)==1) then {
		MAMaS_end = [_title,true];
		publicVariable "MAMaS_end";
	};
	...
	[_title] call MAMaS_processorEND
};
...
if isNil{MAMaS_end} then {
	MAMaS_end = ['',false];
}else{
	if (MAMaS_end select 1) then {
		MAMaS_end call MAMaS_endMission
	};
};
...
if (!isDedicated&&isServer) then {//костыль для тестов
	...
	"MAMaS_end" addPublicVariableEventHandler {
		if ((_this select 1) select 1) then {
			(_this select 1) call MAMaS_endMission
		};
	};
};


MAMaS_all_units = []; //собираем список юнитов в начале игры потому-что узнать имя мертвого игрока нельзя
{
	_show = false;
	_unitsInGroup = [];
	{if isPlayer(_x) then {
	...
	if _show then {
		MAMaS_all_units set [count MAMaS_all_units, [_x,_unitsInGroup]];
	};
} forEach allGroups;
